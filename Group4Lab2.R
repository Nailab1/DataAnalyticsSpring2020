...
##Visualize the behavior of .nknots.smpl()
...
(s2<-smooth.spline(y18,control=list(trace=T, tol=1e-6, low=-1.5)))
(s2m<-smooth.spline(y18,cv=T, control=list(trace=T, tol=1e-6, low=-1.5)))
#lab1_splines2
...
data(cars)
head(cars)
cars.spl<-smooth.spline(cars$speed,cars$dist)
(cars.spl)
install.packages("mlbench")
library(mlbench)
data(Ionosphere)
summary(Ionosphere)
ionosphere<-smooth.spline(Ionosphere$V34, Ionosphere$Class)
data(Glass)
glass.spl<-smooth.spline(Glass$Mg, Glass$Type)
